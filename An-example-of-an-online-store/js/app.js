(() => {
    ...
    let documentClick = e => {
        ...
        if (targetElement.closest(".button-search")) {
            const formSearch = document.querySelector(".search");
            formSearch.classList.toggle("open-search");
        }
        if (document.querySelector(".open-search")) if (!targetElement.closest(".header")) {
            const openSearch = document.querySelector(".open-search");
            openSearch.classList.remove("open-search");
        }
        if (targetElement.closest(".comment__button-actions")) {
            const comment = targetElement.closest(".comment");
            comment.classList.toggle("menu-actions--open");
        }
        if (document.querySelector(".menu-actions--open")) if (!targetElement.closest(".comment")) {
            const openSearch = document.querySelector(".menu-actions--open");
            openSearch.classList.remove("menu-actions--open");
        }
        if (targetElement.closest(".info-header-content-casual__button-filter")) {
            const aside = document.querySelector(".aside");
            const casual = document.querySelector(".casual");
            const body = document.querySelector("body");
            aside.classList.toggle("open-aside");
            casual.classList.toggle("open-aside");
            body.classList.toggle("hidden");
        }
        if (targetElement.closest(".header-filter__cross")) if (document.querySelector(".open-aside")) {
            const openAsides = document.querySelectorAll(".open-aside");
            const body = document.querySelector("body");
            body.classList.remove("hidden");
            openAsides.forEach((openAside => {
                openAside.classList.remove("open-aside");
            }));
        }
    };
    document.addEventListener("click", documentClick);
    if (document.querySelector(".item-price")) {
        const minValue = document.querySelector(".price-range__min-val");
        const maxValue = document.querySelector(".price-range__max-val");
        const minTooltip = document.querySelector(".min-tooltip");
        const maxTooltip = document.querySelector(".max-tooltip");
        const minGap = 500;
        const range = document.querySelector(".price-range__track");
        parseInt(minValue.min);
        const sliderMaxValue = parseInt(maxValue.max);
        function slideMin() {
            let gap = parseInt(maxValue.value) - parseInt(minValue.value);
            if (gap <= minGap) minValue.value = parseInt(maxValue.value) - minGap;
            minTooltip.innerHTML = "$" + minValue.value;
            setArea();
        }
        function slideMax() {
            let gap = parseInt(maxValue.value) - parseInt(minValue.value);
            if (gap <= minGap) maxValue.value = parseInt(minValue.value) + minGap;
    ...
    class SelectConstructor {
        constructor(props, data = null) {
            ...
            this.selectClasses = {
                ...
                classSelectValue: "select__value _icon-mini-arrow",
    ...
    function initSliders() {
        if (document.querySelector(".swiper")) {
            ...
            new swiper_core_Swiper(".shape-intro-product__main-slider", {
                modules: [ Navigation, Autoplay, Thumb, Controller ],
                observer: true,
                observeParents: true,
                slidesPerView: 1,
                spaceBetween: 30,
                ...
                thumbs: {
                    swiper: {
                        el: ".shape-intro-product__mini-slider",
                        slidesPerView: 3,
                        direction: "vertical",
                        breakpoints: {
                            310: {
            ...
            new swiper_core_Swiper(".shape-intro-product__mini-slider", {
                modules: [ Navigation, Autoplay, Thumb, Controller ],
                observer: true,
                observeParents: true,
                slidesPerView: 3,
