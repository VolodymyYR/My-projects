(() => {
    ...
    function EffectFlip(_ref) {
        ...
        const createSlideShadows = (slideEl, progress) => {
            ...
            if (!shadowBefore) shadowBefore = create_shadow_createShadow("flip", slideEl, swiper.isHorizontal() ? "left" : "top");
            if (!shadowAfter) shadowAfter = create_shadow_createShadow("flip", slideEl, swiper.isHorizontal() ? "right" : "bottom");
        ...
        const setTranslate = () => {
            ...
            const rotateFix = utils_getRotateFix(swiper);
            for (let i = 0; i < slides.length; i += 1) {
    ...
    function initSliders() {
        if (document.querySelector(".swiper")) {
            const mainSwiper = new swiper_core_Swiper(".main-circle__slider", {
                modules: [ Navigation, Controller, EffectFlip ],
                observer: true,
                observeParents: true,
                slidesPerView: 1,
                spaceBetween: 50,
                ...
                on: {
                    init: function(slider) {
                        slider.slides.forEach((slide => {
                            const imageSrc = slide.querySelector(".slide-circle__circle-img").getAttribute("src");
                            const imageTop = `\n                            <div class="slide-circle__up-photo">\n                                <img src="${imageSrc}" alt="Image">\n                            </div>\n                        `;
                            slide.insertAdjacentHTML("beforeend", imageTop);
                        }));
                    }
                }
            });
            const miniSwiper = new swiper_core_Swiper(".mini-circle__slider", {
